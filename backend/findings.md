...
- Implement custom exceptions that accept `ErrorMessages.*` and optional `details` (e.g., business validation).
...
- `DecimalTransformInterceptor` defines a different envelope but is registered globally via `APP_INTERCEPTOR`.
...
| Add custom exceptions (base + business validation + not found) | Enforce keyed errors and optional details at compile time |
| Keep current modules unchanged initially; provide migration guide | User requested base structure only |
