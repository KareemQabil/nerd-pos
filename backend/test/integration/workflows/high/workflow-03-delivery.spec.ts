...
describe('Workflow 3: Takeout/Delivery Order', () => {
    ...
    describe('3.1: Delivery Fee Calculation', () => {
        ...
        it('should throw if zone not found', async () => {
            ...
            await expect(service.calculateFee('Unknown', 100))
    ...
    describe('3.2: Create Delivery', () => {
        it('should create delivery with fee', async () => {
            ...
            expect(eventBus.publish).toHaveBeenCalledWith('DeliveryCreated', expect.anything());
        });
    });
    ...
    describe('3.3: Driver Assignment', () => {
        it('should assign available driver', async () => {
            repo.findDriverById.mockResolvedValue({ id: 'driver-1', status: 'AVAILABLE' });
            repo.update.mockResolvedValue({ id: 'del-1', driverId: 'driver-1', status: 'ASSIGNED' });
            ...
            expect(eventBus.publish).toHaveBeenCalledWith('DriverAssigned', expect.anything());
        });

        it('should throw if driver not available', async () => {
            repo.findDriverById.mockResolvedValue({ id: 'driver-1', status: 'BUSY' });

            await expect(service.assignDriver('del-1', 'driver-1'))
                .rejects.toThrow(BadRequestException);
        });
    });
    ...
    describe('3.4: Status Updates', () => {
        it('should update status to PICKED_UP', async () => {
            ...
            expect(eventBus.publish).toHaveBeenCalledWith('DeliveryStatusUpdated', expect.anything());
        });
        ...
        it('should complete delivery and free driver', async () => {
            ...
            expect(repo.updateDriver).toHaveBeenCalledWith('driver-1', { status: 'AVAILABLE' });
            expect(eventBus.publish).toHaveBeenCalledWith('DeliveryCompleted', expect.anything());
        });
    });
    ...
    describe('3.6: Driver Management', () => {
        ...
        it('should update driver location', async () => {
            ...
            expect(repo.updateDriver).toHaveBeenCalledWith('driver-1', expect.objectContaining({
                latitude: 24.7136,
                longitude: 46.6753,
            }));
        });
