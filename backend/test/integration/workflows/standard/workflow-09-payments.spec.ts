...
describe('Workflow 9: Payments & Refunds', () => {
    ...
    describe('9.3: Split Payment', () => {
        it('should process split payment', async () => {
            repo.create
                .mockResolvedValueOnce({ id: 'pay-1', method: 'CASH', amount: 50, status: 'COMPLETED' })
                .mockResolvedValueOnce({ id: 'pay-2', method: 'CARD', amount: 50, status: 'COMPLETED' });
    ...
    describe('9.4: Refund Creation', () => {
        ...
        it('should require approval for large refunds (>= 100 SAR)', async () => {
            repo.findById.mockResolvedValue({ id: 'pay-1', amount: 500, refundedAmount: 0 });
            repo.createRefund.mockResolvedValue({
                id: 'ref-1',
                amount: 200,
                status: 'PENDING',
        ...
        it('should throw if refund exceeds payment', async () => {
            repo.findById.mockResolvedValue({ id: 'pay-1', amount: 100, refundedAmount: 50 });
    ...
    describe('9.5: Refund Approval', () => {
        ...
        it('should reject refund', async () => {
            ...
            const result = await service.rejectRefund('ref-1', 'manager-1', 'No valid reason');
