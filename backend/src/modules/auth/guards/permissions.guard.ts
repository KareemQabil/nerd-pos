...
export class PermissionsGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        const hasAllPermissions = await this.checkPermissions(user.sub, requiredPermissions);
    ...
    private async checkPermissions(userId: string, requiredPermissions: string[]): Promise<boolean> {
        for (const permissionCode of requiredPermissions) {
            const hasPermission = await this.usersService.hasPermission(userId, permissionCode);
            if (!hasPermission) {
                return false;
            }
        }
