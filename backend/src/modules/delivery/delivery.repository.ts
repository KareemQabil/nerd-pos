...
export class DeliveryRepository extends BaseRepository<Delivery> {
    ...
    protected get model() { return 'delivery'; }
    ...
    async findActive(driverId?: string): Promise<Delivery[]> {
        const where: any = { status: { in: ['PENDING', 'ASSIGNED', 'PICKED_UP', 'IN_TRANSIT'] } };
        if (driverId) where.driverId = driverId;
        return (this.prisma as any).delivery.findMany({ where, orderBy: { createdAt: 'asc' } });
    }

    // Zones
    async findAllZones(): Promise<DeliveryZone[]> {
        return (this.prisma as any).deliveryZone.findMany({ where: { isActive: true } });
    }

    async findZoneByDistrict(district: string): Promise<DeliveryZone | null> {
        return (this.prisma as any).deliveryZone.findFirst({ where: { districts: { has: district }, isActive: true } });
    }

    async createZone(data: any): Promise<DeliveryZone> {
        return (this.prisma as any).deliveryZone.create({ data: { ...data, isActive: true } });
    }
    ...
    async findAvailableDrivers(): Promise<Driver[]> {
        return (this.prisma as any).driver.findMany({ where: { status: 'AVAILABLE', isActive: true } });
    }
    ...
    async createDriver(data: any): Promise<Driver> {
        return (this.prisma as any).driver.create({ data: { ...data, status: 'OFFLINE', totalDeliveries: 0, isActive: true } });
    }
