...
import { InvoiceGeneratedEvent, InvoiceSubmittedEvent, HashChainBrokenEvent } from './events/compliance.events';
...
export class ComplianceService {
    ...
    async generateInvoice(orderId: string, orderData: any): Promise<ZATCAInvoice> {
        ...
        const invoice = await this.repo.create({
            orderId, invoiceNumber, uuid: uuidv4(),
            previousHash, currentHash, invoiceXML, qrCode,
            submissionStatus: 'PENDING',
        });

        await this.eventBus.publish('InvoiceGenerated', new InvoiceGeneratedEvent(invoice.id, orderId));
        return invoice;
    }
    ...
    private calculateHash(previousHash: string, xml: string): string {
        return crypto.createHash('sha256').update(previousHash + xml).digest('hex');
    }

    private generateQRCode(orderData: any, hash: string): string {
        // Base64 encoded TLV per ZATCA spec
        return Buffer.from(JSON.stringify({ hash: hash.slice(0, 16), total: orderData.grandTotal })).toString('base64');
    }
    ...
    async submitInvoice(invoiceId: string): Promise<ZATCAInvoice> {
        ...
        const updated = await this.repo.update(invoiceId, {
            submissionStatus: 'ACCEPTED', submittedAt: new Date(),
            responseCode: '200', responseMessage: 'Accepted',
        });

        await this.eventBus.publish('InvoiceSubmitted', new InvoiceSubmittedEvent(invoiceId, 'ACCEPTED'));
        return updated;
    }
    ...
    async getPendingInvoices(): Promise<ZATCAInvoice[]> { return this.repo.findPending(); }
    async findByOrder(orderId: string): Promise<ZATCAInvoice | null> { return this.repo.findByOrder(orderId); }
}
