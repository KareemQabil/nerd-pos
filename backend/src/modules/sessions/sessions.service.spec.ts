...
describe('SessionsService', () => {
    ...
    describe('openSession', () => {
        it('should open session with opening balance', async () => {
            ...
            expect(eventBus.publish).toHaveBeenCalledWith('SessionOpened', expect.anything());
        });
    ...
    describe('closeSession', () => {
        it('should close session with denomination count', async () => {
            ...
            repo.update.mockResolvedValue(closedSession);

            const result = await service.closeSession(dto);

            expect(result.status).toBe('CLOSED');
            expect(repo.createDenomination).toHaveBeenCalledTimes(3);
            expect(eventBus.publish).toHaveBeenCalledWith('SessionClosed', expect.anything());
        });
        ...
        it('should publish variance alert for large variance', async () => {
            ...
            expect(eventBus.publish).toHaveBeenCalledWith('SessionVarianceAlert', expect.anything());
        });
    });
    ...
    describe('updateSessionStats', () => {
        it('should update running session totals', async () => {
            ...
            expect(repo.update).toHaveBeenCalledWith('session-1', expect.objectContaining({
                totalSales: 175,
                totalCash: 75,
                totalCard: 100,
                orderCount: 6,
