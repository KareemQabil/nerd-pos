...
import { Discount, DiscountUsage, DiscountValidationResult } from './entities/discounts.entity';
...
export class DiscountsService {
    ...
    async validateAndCalculate(
        ...
        if (discount.maxUsesPerCustomer && customerId) {
            const customerUsage = await this.repo.countUsageByCustomer(discount.id, customerId);
        ...
        const requiresApproval =
            ...
            discountAmount.greaterThanOrEqualTo(new Decimal(discount.approvalThreshold));
    ...
    async applyDiscount(dto: ApplyDiscountDto): Promise<DiscountUsage> {
        ...
        if (discount && discount.maxUses && discount.usedCount + 1 >= discount.maxUses) {
            await this.eventBus.publish(
