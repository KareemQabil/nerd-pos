...
describe('ReportsService', () => {
    ...
    describe('generateDailySalesReport', () => {
        it('should calculate total sales and tax using Decimal.js', async () => {
            ...
            const result = await service.generateDailySalesReport(new Date('2026-01-18'));
        ...
        it('should handle orders with null grandTotal', async () => {
            prisma.order.findMany.mockResolvedValue([
                { id: 'order-1', grandTotal: null, taxAmount: null, status: 'COMPLETED' },
            ]);
    ...
    describe('getInventoryValuation', () => {
        it('should calculate total inventory value using Decimal.js', async () => {
            prisma.inventoryItem.findMany.mockResolvedValue([
                { productId: 'prod-1', quantity: 100, cost: 10, product: { name: 'Chicken' } },
                { productId: 'prod-2', quantity: 50, cost: 20, product: { name: 'Beef' } },
            ]);
        ...
        it('should handle null quantity or cost', async () => {
            prisma.inventoryItem.findMany.mockResolvedValue([
                { productId: 'prod-1', quantity: null, cost: null, product: { name: 'Test' } },
            ]);
        ...
        it('should include product name in valuation', async () => {
            prisma.inventoryItem.findMany.mockResolvedValue([
                { productId: 'prod-1', quantity: 10, cost: 5, product: { name: 'Chicken Wings' } },
            ]);
