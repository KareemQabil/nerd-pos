...
const mockFloor = { id: 'floor-1', name: 'Ground Floor', nameAr: 'الطابق الأرضي', sortOrder: 1, isActive: true };
const mockTable = { id: 'table-1', number: 'T1', floorId: 'floor-1', capacity: 4, status: 'AVAILABLE', isActive: true };
...
describe('TablesService', () => {
    ...
    describe('getAllFloors', () => {
        it('should return all floors', async () => {
            repo.findAllFloors.mockResolvedValue([mockFloor, { ...mockFloor, id: 'floor-2', name: 'Terrace' }]);
    ...
    describe('getFloorWithTables', () => {
        it('should return floor with tables', async () => {
            repo.findFloorWithTables.mockResolvedValue({ ...mockFloor, tables: [mockTable] });
        ...
        it('should throw NotFoundException if floor not found', async () => {
            ...
            await expect(service.getFloorWithTables('invalid')).rejects.toThrow(NotFoundException);
        });
    });

    describe('createFloor', () => {
        it('should create floor with defaults', async () => {
            repo.createFloor.mockImplementation((data) => Promise.resolve({ id: 'new', ...data }));
            await service.createFloor({ name: 'New Floor', nameAr: 'طابق جديد', displayOrder: 1 });
            expect(repo.createFloor).toHaveBeenCalledWith(expect.objectContaining({ isActive: true }));
        });
    });
    ...
    describe('createTable', () => {
        it('should create table with defaults', async () => {
            ...
            await service.createTable({ number: 'T5', floorId: 'floor-1', capacity: 4, section: 'INDOOR' });
    ...
    describe('assignOrderToTable', () => {
        it('should assign order and publish TableOccupied event', async () => {
            ...
            repo.update.mockResolvedValue({ ...mockTable, status: 'OCCUPIED', currentOrderId: 'order-1' });
        ...
        it('should throw NotFoundException if table not found', async () => {
            ...
            await expect(service.assignOrderToTable('invalid', 'order-1')).rejects.toThrow(NotFoundException);
        });
        ...
        it('should throw BadRequestException if table not available', async () => {
            ...
            await expect(service.assignOrderToTable('table-1', 'order-1')).rejects.toThrow(BadRequestException);
        });
    });
    ...
    describe('releaseTable', () => {
        it('should release table and publish TableReleased event', async () => {
            repo.findById.mockResolvedValue({ ...mockTable, status: 'OCCUPIED' });
            repo.update.mockResolvedValue({ ...mockTable, status: 'DIRTY', currentOrderId: null });
    ...
    describe('transferTable', () => {
        it('should transfer order between tables and publish event', async () => {
            const fromTable = { ...mockTable, id: 'from', status: 'OCCUPIED', currentOrderId: 'order-1' };
            const toTable = { ...mockTable, id: 'to', number: 'T2', status: 'AVAILABLE' };

            repo.findById.mockResolvedValueOnce(fromTable).mockResolvedValueOnce(toTable);
            repo.update.mockResolvedValue(toTable);

            await service.transferTable({ fromTableId: 'from', toTableId: 'to', orderId: 'order-1' });
        ...
        it('should throw BadRequestException if target table not available', async () => {
            repo.findById.mockResolvedValueOnce(mockTable).mockResolvedValueOnce({ ...mockTable, status: 'OCCUPIED' });
            await expect(service.transferTable({
                fromTableId: 'from', toTableId: 'to', orderId: 'order-1',
    ...
    describe('createReservation', () => {
        it('should create reservation and publish event', async () => {
            ...
            repo.createReservation.mockResolvedValue({
                id: 'res-1', tableId: 'table-1', status: 'PENDING',
            });
            ...
            expect(eventBus.publish).toHaveBeenCalledWith('ReservationCreated', expect.anything());
        });
    ...
    describe('updateReservationStatus', () => {
        it('should update status and publish event', async () => {
            repo.updateReservation.mockResolvedValue({ id: 'res-1', tableId: 'table-1', status: 'CONFIRMED' });
            repo.update.mockResolvedValue(mockTable);

            const result = await service.updateReservationStatus('res-1', { status: 'CONFIRMED' });

            expect(result.status).toBe('CONFIRMED');
            expect(eventBus.publish).toHaveBeenCalledWith('ReservationStatusChanged', expect.anything());
        });
    });
});
