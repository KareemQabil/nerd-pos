...
export class FIFOStrategy {
    ...
    async deduct(
        ...
        if (!item) {
            throw new BadRequestException(`No inventory for product ${productId} in warehouse ${warehouseId}`);
        }
    ...
    async getAvailableStock(productId: string, warehouseId: string): Promise<number> {
        ...
        const available = new Decimal(item.quantityOnHand).minus(item.quantityReserved);
        return available.toNumber();
    }

    async getCOGS(productId: string, warehouseId: string, quantity: number): Promise<Decimal> {
        ...
        for (const batch of batches) {
            ...
            const deductQty = Decimal.min(remaining, new Decimal(batch.quantityRemaining));
            const unitCost = new Decimal(batch.costPerUnit);
